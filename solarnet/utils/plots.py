...
def plot_images(images,
    ...
    # plot all time steps
    pdf = PdfPages(save_path / "test.pdf")
    labels = GetLabels(y_flux)
    true_labels = [0, 1, 2, 3]
    for label in true_labels:
        indixes = indices(labels, label)
        x={}
        for i in indixes:
            x[i] = y_proba[i][0]
        new_dic = {k: v for k, v in sorted(x.items(), key=lambda item: item[1])}
        ...
        color_g = 'green'
        count=0
        for idx in list(new_dic.keys()):
            fig = plt.figure(figsize=(30, 10))
            dr = 1
            dc = 4

            for ts in range(4):
                plt.subplot(dr, dc, ts+1)

                plt.imshow(images[idx][0][ts].squeeze(), vmin=-1, vmax=1)
                plt.axis("off")

                l = y_proba[idx]
                color = color_g if y_pred[idx] == 0 else color_r
                if ts ==0:
                    plt.title('prediction:{}'.format(l), color = color , fontsize = 25)
            if count == 0:
                plt.colorbar(orientation="vertical")
            fig.suptitle("class: non-flare", fontsize=25)
...
def plot_test_curve(
    ...
    plt.plot(metrics, label='Test tss')
    #plt.plot(best_epoch, metrics[best_epoch],marker="o", color= "red" )
    plt.grid(linestyle='dotted')
    plt.title('best_epoch: {}, tss: {}, max_tss: {}'.format(best_epoch, tss, max(metrics)))
    plt.legend()
