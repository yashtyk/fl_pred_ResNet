...
from solarnet.models.classifier import Classifier, Classifier1, Classifier2, Classifier3
...
class ImageClassification_multi_deep(BaseModel):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        for chan in channels_list:
            if isinstance(backbone, str):
                ...
                **kwargs,
                ...
                self.backbones[chan] = backbone_chan.to(device)
        self.optimizer = optimizer
        if classifier_type == 1:
            classifier = Classifier(backbone_output_size*len(channels_list), n_class, n_hidden, dropout)
        if classifier_type ==2:
            classifier = Classifier1(backbone_output_size*len(channels_list), n_class, n_hidden, dropout)
        if classifier_type == 3:
            classifier = Classifier(backbone_output_size * len(channels_list), n_class, 512, dropout)
        if classifier_type ==4:
            classifier = Classifier2(backbone_output_size*len(channels_list), n_class, n_hidden, dropout)
        if classifier_type ==5:
            classifier = Classifier3(backbone_output_size * len(channels_list), n_class, n_hidden, dropout)
        ...
        lparam+=list(self.classifier.parameters())
    ...
    def forward(self, image):
        ...
        #concatanate outputs
        latent = torch.cat([chan_outs[chan] for chan in self.channels_list], axis =1)
